...
channelController.getChannel = (req, res) => {
  let userId;
  if (req.user) { // Set id if user is logged in.
    userId = req.user.id;
  }

  const channelURL = req.params.channelURL.toLowerCase().trim().replace(/\s+/g, '_');
  ...
  db.Channel.findOne({
    ...
    attributes: ['id', 'name', 'channelURL', 'desc', 'businessEmail', 'userId', 'searchId', 'color'],
  }).then((channel) => {
    if (!channel) {
    ...
    db.User.findOne({ where: { id: channel.userId }, attributes: ['avatarPath'] })
    .then((userAvatar) => {
...
channelController.createChannel = (req, res) => {
  ...
  const channelURL = name.toLowerCase().trim().replace(/\s+/g, '_');
...
channelController.getAllFeatured = (req, res) => {
  db.Channel.findAll({
    ...
    attributes: ['id', 'name', 'desc', 'userId', 'color', 'channelURL', [db.sequelize.fn('COUNT', db.sequelize.col('UserSubscriptions.id')), 'subscribers']],
    include: [
      {
        model: db.User,
        attributes: ['avatarPath'],
